...
func runDaemon(cfg *Config) error {
	...
	for {
		...
		pm := NewProgressMonitor(
			filepath.Join(cfg.CWD, ".ralph-tui", "progress.md"),
			filepath.Join(cfg.CWD, ".ralph-tui", "session.json"),
		)
		stuckCh := make(chan struct{}, 1)
		stopMonitor := make(chan struct{})

		// Grace period before stuck detection starts
		...
		go func() {
			for {
				...
				case <-time.After(time.Second):
					// Skip stuck check during grace period
...
// Watches multiple files to get a more accurate picture of activity.
...
func NewProgressMonitor(paths ...string) *ProgressMonitor {
	pm := &ProgressMonitor{paths: paths, lastMods: make(map[string]time.Time)}
	for _, p := range paths {
		if info, err := os.Stat(p); err == nil {
			pm.lastMods[p] = info.ModTime()
		}
	}
	return pm
}
...
func (pm *ProgressMonitor) Check() time.Duration {
	pm.mu.Lock()
	defer pm.mu.Unlock()
	var maxStuck time.Duration
	for _, p := range pm.paths {
		if info, err := os.Stat(p); err == nil {
			if info.ModTime().After(pm.lastMods[p]) {
				pm.lastMods[p] = info.ModTime()
			} else {
				stuck := time.Since(pm.lastMods[p])
				if stuck > maxStuck {
					maxStuck = stuck
				}
			}
