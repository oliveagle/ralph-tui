...
Gemini CLI requires authentication with Google Cloud. You are responsible for configuring authentication before using this plugin.

**Google Cloud authentication** (recommended):
```bash
...
Authentication is required for Gemini CLI to function. Ralph TUI does not manage Google Cloud credentials or API keys - you must configure them separately.
</Callout>
...
gemini-cli \
  ...
  --include-directories /path \    # For file context
  < prompt.txt                     # Prompt via stdin
```

## Authentication

Gemini CLI requires Google Cloud authentication or an API key. You are responsible for configuring this before using the plugin.

**Options:**
- Google Cloud Application Default Credentials: `gcloud auth application-default login`
- API key: `gemini-cli config set api-key YOUR_API_KEY`
